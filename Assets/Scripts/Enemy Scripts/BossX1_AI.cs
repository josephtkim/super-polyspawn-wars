...
public class BossX1_AI : MonoBehaviour
{
    GameObject player;
    ...
    private void Update()
    {
        ...
        {
            if (hitpoints < 900) // At hp threshold enter charing movement pattern
            {
                if (isFury == false)
                {
    ...
    {
        yield return new WaitForSeconds(1.5f); // 2 delay before fury mode attacks
        ...
        while (true)
        {
            if (isMoving) // Move normally
            {
                ...
                int randomAttack = Random.Range(0, 5); // 40% chance of cross Attack                
                ...
                {                    
                    isCrossing = true;
                    // Move the boss to the top left to start charging                    
    ...
    {
        gameObject.GetComponent<Rigidbody2D>().velocity = new Vector2(prevDir.x, prevDir.y) * 4.1f;
