...
public class Boss4_AI : MonoBehaviour
{
    GameObject player;
    ...
    private void Update()
    {
        ...
        {
            if (hitpoints < 700) // At hp threshold enter charing movement pattern
            {
                if (isFury == false)
                {
    ...
    IEnumerator ChargingMovementRoutine()
    {
        ...
        {            
            if (isMoving) // Move normally
            {
                ...
                int randomAttack = Random.Range(0, 5); // 20% chance of cross Attack                
                ...
                {                 
                    isCrossing = true;
                    // Move the boss to the left first to start cross attack              
            ...
            {             
                if (isCrossing)
                {
                    // Move boss to the top to start second part of cross attack                    
    ...
    {
        gameObject.GetComponent<Rigidbody2D>().velocity = new Vector2(prevDir.x, prevDir.y) * 3.2f;
